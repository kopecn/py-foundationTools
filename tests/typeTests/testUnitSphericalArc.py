...
class TestUnitSphericalArc(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.short_arc = UnitSphericalArc(
            arc_length=math.pi / 4, azimuth=0.0, polar=math.pi / 6
        )
        self.semicircle_arc = UnitSphericalArc(
            arc_length=math.pi, azimuth=math.pi / 2, polar=math.pi / 3
        )
        self.arbitrary_arc = UnitSphericalArc(arc_length=1.5, azimuth=2.0, polar=0.8)

    def test_from_dict_creates_valid_instance(self):
        """Test that from_dict creates a valid UnitSphericalArc instance."""
        test_data = {"arcLength": 1.0, "azimuth": 0.5, "polar": 0.3}
    ...
    def test_roundtrip_data_integrity(self):
        ...
        test_arcs = [
            ...
            self.semicircle_arc,
            self.arbitrary_arc,
            UnitSphericalArc(arc_length=0.0, azimuth=0.0, polar=0.0),
            UnitSphericalArc(
                arc_length=2 * math.pi, azimuth=2 * math.pi, polar=math.pi
            ),
            UnitSphericalArc(arc_length=-math.pi, azimuth=math.pi, polar=math.pi / 2),
        ]
    ...
    def test_from_dict_validates_types(self):
        ...
        invalid_data = {"arcLength": "not a number", "azimuth": 0.5, "polar": 0.3}
    ...
    def test_from_dict_accepts_integers(self):
        ...
        test_data = {"arcLength": 1, "azimuth": 2, "polar": 3}
        arc = UnitSphericalArc.from_dict(test_data)
    ...
    def test_to_dict_preserves_numeric_types(self):
        ...
        arc = UnitSphericalArc(arc_length=1.5, azimuth=2.5, polar=3.5)
        result = arc.to_dict()
