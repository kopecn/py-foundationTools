...
- `run_ruff` applies ruff format + `--fix`, whose `UP` rules modernize annotations
  (e.g. `Type[T]` → `type[T]`, `Optional[X]` → `X | None`).
...
One schema → one script → one generated module.
...
- **Field ordering.** quicktype can emit a required (non-default) dataclass field after
  ...
  does not yet auto-correct this; `ModelContextProtocol.py` required a manual default.
  A required-after-default field is a signal the schema or pipeline needs adjustment.
- **Non-conforming scripts.** `schema/scripts/generateGeoCoordinate.sh` does **not**
  source the shared libraries and produces non-strict output; it SHALL be migrated to
  the golden workflow.
