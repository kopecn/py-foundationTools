...
2. Sweep for stale references: `grep -ri "foundationCLIHelpers" .` must return
   nothing; every `applies_to:` path must exist.
...
- [ ] Zero `foundationCLIHelpers` references repo-wide.
